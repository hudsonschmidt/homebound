...
async def public_checkin(token: str, request: Request, session: AsyncSession = Depends(get_db)):
    try:
...
async def public_checkout(token: str, request: Request, session: AsyncSession = Depends(get_db)):
    try:
